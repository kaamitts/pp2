...
try:
    ...
    if result:
        ...
        print(f"Добро пожаловать обратно, {username}! Ваш текущий уровень - {current_level}.")
    else:
        # Если пользователь не существует, добавить его в базу данных
        c.execute("INSERT INTO users (username) VALUES (?)", (username,))
        conn.commit()
        current_level = 1
        print(f"Добро пожаловать, {username}! Начнем на уровне {current_level}.")

    # Этот класс помогает нам с управлением и проверками границ
    class SnakeBlock:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                # Само движение
                    ...
                    # Приостановка игры и сохранение текущего состояния
                    c.execute("UPDATE users SET level = ?, score = ? WHERE username = ?", (current_level, total, username))
                    conn.commit()
                    print("Игра приостановлена. Ваш прогресс сохранен.")
                    # Добавьте функциональность паузы здесь
